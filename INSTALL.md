# Building and Installing the Crossplane Existing Clusters Stack

`provider-existing-cluster` is composed of a golang project and can be built directly with standard `golang` tools. We currently support two different platforms for building:
...
You can build the Crossplane GCP Stack for the host platform by simply running the command below.
...
TBD: Steps to install the GCP stack into a Crossplane cluster
